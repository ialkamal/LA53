...
std::vector<std::string> Split(std::string whole, std::string separator){
    ...
    do{
        int pos = whole.find(separator);
        vec.push_back(whole.substr(0, pos));
        whole.erase(0, pos + separator.length());
    } while (whole.find(separator) != std::string::npos);
...
std::string Join(std::vector<std::string> pieces, std::string glue){
    ...
    result.erase(result.length()-glue.length(),glue.length());
    return result;
}
...
std::vector<int> MatchVectors(std::vector<int> a, std::vector<int> b){
    ...
    for (int i=0;i<static_cast<int>(a.size());i++) 
        if (std::find(b.begin(),b.end(),a[i]) == b.end()) result.push_back(a[i]);
...
//     std::vector<std::string> result = Split("Hi!  my name is Ismail.", "");
...
//     std::cout << RemoveAllSubstrings("Welcome in the center in the welcoming Las Vegas!", " the") << std::endl;

//     std::cout << RemoveFirstSubstring("Welcome in the center in the welcoming Las Vegas!", " the") << std::endl;
...
//     std::vector<int> res_ints = MatchVectors(a,b);

//     std::cout << "Result: ";
    
//     for (auto num : res_ints) {
